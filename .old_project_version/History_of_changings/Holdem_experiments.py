...
def bet_comp_function(round_poker):
    global combination_human, combination_comp, opponent_money, my_money, winer, low, whose_stack_is_first, bank, \
        bet_human, bet_comp, human, comp, comp_fold
    ...
    if round_poker == 'префлоп':
        if bet_human < opponent_money:
            if (kard1_comp[1] == kard2_comp[1] or kard1_comp[0] == kard2_comp[0]) and \
                ...
                bet_comp = 2 * bet_human
                if int(max_comp) - int(min_comp) == 1:
                    bet_comp = 4 * bet_human
                ...
                if int(max_comp) - int(min_comp) == 1:
                    ...
                    if random.randint(0, 1) == 0:
                        bet_comp = bet_comp = bet_human + random.randint(10, 30)
                    else:
                        bet_comp = bet_human
            elif max_comp > max_human:
                if random.randint(0, 1) == 1:
                    bet_comp = bet_human + 10
                else:
                    bet_comp = bet_human
        ...
        if bet_human != 0:
            if random.randint(0, 20) == 10:
                bet_comp = 75 * bet_human
            else:
                if int(vocabulary_combo[combination_comp]) - int(vocabulary_combo[combination_human]) >= 2:
                    bet_comp = 3 * bet_human
                elif int(vocabulary_combo[combination_comp]) == int(vocabulary_combo[combination_human]):
                    if random.randint(0, 3) > 0:
                        bet_comp = bet_human + random.randint(0, 10)
                    else:
                        bet_comp = bet_human
                elif kard1_comp[1] == kard2_comp[1] or kard1_comp[0] == kard2_comp[0]:
                    bet_comp = bet_human + random.randint(10, 30)
                elif winer == 'comp':
                    if int(max_comp) > int(max_human):
                        bet_comp = bet_human + random.randint(0, 20)
                    elif int(min_comp) > int(min_human):
                        bet_comp = bet_human + random.randint(0, 10)
                    elif int(max_comp) - int(min_comp) == 1:
                        bet_comp = bet_human + random.randint(0, 5)
                    elif (int(max_comp) <= int(max_human)) and (int(min_comp) <= int(min_human)):
                        bet_comp = bet_human
                elif winer == 'human':
                    if int(max_comp) > int(max_human):
                        bet_comp = bet_human + random.randint(0, 5)
                        ...
                        bet_comp = bet_human + 5
                else:
                    comp_fold = True
        else:
            if random.randint(0, 20) == 10:
                bet_comp = random.randint(400, 750)
            else:
                if int(vocabulary_combo[combination_comp]) - int(vocabulary_combo[combination_human]) >= 2:
                    bet_comp = random.randint(100, 250)
                elif int(vocabulary_combo[combination_comp]) == int(vocabulary_combo[combination_human]):
                    if random.randint(0, 3) > 0:
                        bet_comp = random.randint(0, 30)
                    else:
                        bet_comp = bet_human
                elif kard1_comp[1] == kard2_comp[1] or kard1_comp[0] == kard2_comp[0]:
                    bet_comp = random.randint(20, 40)
                elif winer == 'comp':
                    if int(max_comp) > int(max_human):
                        bet_comp = random.randint(0, 20)
                    elif int(min_comp) > int(min_human):
                        bet_comp = random.randint(0, 10)
                    elif int(max_comp) - int(min_comp) == 1:
                        bet_comp = random.randint(0, 5)
                    elif (int(max_comp) <= int(max_human)) and (int(min_comp) <= int(min_human)):
                        bet_comp = bet_human
                elif winer == 'human':
                    if int(max_comp) > int(max_human):
...
while True:
    print()
    while True:
        game = input('Хотите сыграть партию?   ')
