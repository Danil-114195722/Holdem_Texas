    ...
    if round_poker == 'префлоп':
        ...
        if bet_human != 0:
            if (combination_human == 'royal_flash' or combination_human == 'strit_flash'
                or combination_human == 'four_of_a_kind') or (bet_human >= 500 and combination_comp == 'high_kard'
                                                              and (bet_in_round == 1 or bet_in_round == 2)):
                comp_fold = True
            if not comp_fold:
                ...
                if int(vocabulary_combo[combination_comp]) == int(vocabulary_combo[combination_human]) and \
                    ...
                    bet_comp = bet_human + random.randint(10, 50)
                if int(min_comp) > int(min_human):
                    bet_comp = bet_human + random.randint(25, 75)
                if int(max_comp) > int(max_human):
                    bet_comp = bet_human + random.randint(50, 100)
                if int(max_comp) == int(min_comp):
                    bet_comp = bet_human + random.randint(65, 130)
                if kard1_comp[1] == kard2_comp[1]:
                    bet_comp = bet_human + random.randint(75, 150)
                if int(vocabulary_combo[combination_comp]) == int(vocabulary_combo[combination_human]) and \
                        combination_comp != 'high_kard':
                    bet_comp = bet_human + random.randint(100, 250)
                if int(vocabulary_combo[combination_comp]) - int(vocabulary_combo[combination_human]) >= 2:
                    bet_comp = 30 * bet_human
                if random.randint(0, 10) == 5:
                    bet_comp = 70 * bet_human
                if combination_comp == 'royal_flash' or combination_comp == 'strit_flash':
                    bet_comp = opponent_money
                if bet_in_round == random.randint(2, 4):
                    bet_comp = bet_human
        else:
            if (combination_human == 'royal_flash' or combination_human == 'strit_flash'
                or combination_human == 'four_of_a_kind') or (bet_human >= 500 and combination_comp == 'high_kard'
            ...
            if not comp_fold:
                ...
                if bet_in_round == random.randint(2, 4):
                    bet_comp = bet_human
        if bet_comp < bet_human and not comp_fold:
            bet_comp = bet_human
        if bet_comp > opponent_money:
            bet_comp = opponent_money
        if my_money - bet_human == 0:
...
def bets(round_poker):
    global combination_human, combination_comp, opponent_money, my_money, winer, low, whose_stack_is_first, bank, \
        bet_human, bet_comp, comp_fold, bet_in_round
