...
def bet_comp_function(round_poker):
    ...
    if round_poker == 'префлоп':
        if (kard1_comp[1] == kard2_comp[1] or kard1_comp[0] == kard2_comp[0]) and (bet_comp == 20 or bet_comp == 10):
            bet_comp = 2 * bet_human
            if int(max_comp) - int(min_comp) == 1:
                bet_comp = 4 * bet_human
        elif (kard1_comp[1] == kard2_comp[1] or kard1_comp[0] == kard2_comp[0]) and (bet_comp != 20 or bet_comp != 10):
            if int(max_comp) - int(min_comp) == 1:
                bet_comp = 2 * bet_human
            else:
...
def if_fold(fold_object):
    global opponent_money, my_money, winer, low, whose_stack_is_first, bank, \
        bet_human, bet_comp
...
def bets(round_poker):
    ...
    if round_poker == 'префлоп':
        ...
        while True:
            ...
            if whose_stack_is_first == 'human':
                while True:
                    bet_human = input('Ваша ставка: ')
                    if bet_human.isnumeric() == True and bet_comp <= int(bet_human) <= my_money:
                        bet_human = int(bet_human)
                        break
